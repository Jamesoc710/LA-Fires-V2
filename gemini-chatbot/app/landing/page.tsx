...
// Dark, story-driven landing page matching the chat's warm-black + ember system.
// Content follows LANDING_PAGE_CONTEXT.md: metrics first, then mission/trust,
// then technical capability, then how-to. Serves homeowners and technical
// readers with one page. Do not add claims beyond that doc.
...
export default function LandingPage() {
  return (
    <main className="min-h-screen bg-stone-950 text-stone-100 antialiased">
      {/* nav */}
      <header className="sticky top-0 z-20 border-b border-white/10 bg-stone-950/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <Link
            href="/chat"
            className="whitespace-nowrap rounded-lg bg-stone-100 px-4 py-2 text-sm font-medium text-stone-950 transition hover:bg-white"
          >
      ...
      <section className="relative overflow-hidden">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 top-0 h-[520px] bg-gradient-to-b from-amber-500/[0.07] via-stone-950 to-stone-950"
        />
          ...
          <div>
            <Reveal>
              <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-amber-400/25 bg-amber-400/10 px-3 py-1 text-xs font-medium text-amber-300">
                Free · Built after the January 2025 fires
              </p>
            </Reveal>
            <Reveal delay={100}>
            ...
            <Reveal delay={300}>
              <div className="mt-8 flex flex-wrap items-center gap-3">
                <Link
                  href="/chat"
                  className="rounded-lg bg-stone-100 px-5 py-3 text-sm font-semibold text-stone-950 shadow-sm transition hover:bg-white"
                >
                  Look up your property
                </Link>
                <a
                  href="#how"
                  className="rounded-lg px-5 py-3 text-sm font-semibold text-stone-300 transition hover:bg-white/5"
                >
                  How it works ↓
                </a>
              </div>
            ...
            <Reveal delay={400}>
              <p className="mt-6 text-xs text-stone-500">
                Informational only, not an official determination · Every answer
        ...
        {/* stat band: priority #1, directly under the hero */}
        <div className="relative mx-auto max-w-6xl px-6 pb-20">
          <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-4">
            {[
              ...
              <div key={s.label} className="bg-stone-950 p-6 sm:p-8">
                <Reveal delay={i * 100}>
                  <div className="font-mono text-2xl font-semibold tabular-nums tracking-tight text-amber-300 sm:text-3xl">
      ...
      {/* the problem */}
      <section id="why" className="border-t border-white/10 bg-white/[0.02]">
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
            <Reveal>
              <h2 className="text-3xl font-semibold tracking-tight">
                One question.
                <br />
                <span className="text-stone-500">Five government portals.</span>
              </h2>
            </Reveal>
            <Reveal delay={150}>
        ...
        <div className="mt-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {STEPS.map((step, i) => (
            <Reveal key={step.n} delay={i * 120}>
              <div className="relative border-t-2 border-stone-100 pt-6">
                <span className="font-mono text-sm font-semibold tabular-nums text-stone-500">
                  {step.n}
                </span>
      ...
      <section className="mx-auto max-w-4xl px-6 py-20 sm:py-24 text-center">
        ...
        <Reveal delay={150}>
          <div className="mt-10 overflow-hidden rounded-2xl border border-white/10 bg-stone-900 shadow-lg shadow-black/40">
      ...
      <section className="border-t border-white/10 bg-white/[0.02]">
        <div className="mx-auto max-w-6xl px-6 py-20 sm:py-24">
          ...
          <dl className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/10 sm:grid-cols-2 lg:grid-cols-3">
            {UNDER_THE_HOOD.map((item, i) => (
              <div key={item.k} className="bg-stone-950 p-6">
                <Reveal delay={i * 60}>
      ...
      <section className="mx-auto max-w-6xl px-6 py-20 sm:py-24 text-center">
        <Reveal>
          ...
          <Link
            href="/chat"
            className="mt-8 inline-block rounded-lg bg-stone-100 px-6 py-3.5 text-sm font-semibold text-stone-950 shadow-sm transition hover:bg-white"
          >
            Look up your property
          </Link>
        </Reveal>
